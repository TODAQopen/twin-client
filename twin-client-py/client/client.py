...
class TwinClient:
    ...
    def micropay(
        self, url, token_type_hash, amount, method="get", paywall_path="", data=None
    ):
        ...
        if token_type_hash != paywall_config["targetPayType"]:
            raise TwinMicropayTokenMismatchError(
                f'paywall requires payment of token {paywall_config["targetPayType"]}; attempted to send {token_type_hash}'
            )
        if amount != paywall_config["targetPayQuantity"]:
            raise TwinMicropayAmountMismatchError(
